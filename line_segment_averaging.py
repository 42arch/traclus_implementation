...
def interpolate_within_line_segment(line_segment, horizontal_coordinate):
    """ 计算一个水平坐标x, 在一个线段上的垂直坐标 y
    :param line_segment:
    :param horizontal_coordinate:
    :return: 垂直坐标
    """
    min_x = min(line_segment.start.x, line_segment.end.x)
    max_x = max(line_segment.start.x, line_segment.end.x)

    if not (min_x <= horizontal_coordinate + DECIMAL_MAX_DIFF_FOR_EQUALITY and max_x >= horizontal_coordinate - DECIMAL_MAX_DIFF_FOR_EQUALITY):
        raise Exception(
            "horizontal coordinate " + str(horizontal_coordinate) + " not within horizontal range of line segment" +
...
def get_mean_vertical_coordinate_in_line_segment(line_segments_to_average):
    def apply_interpolation_to_line_segment(interpolation_info):
        if interpolation_info['line_seg'] is None or interpolation_info['horizontal_pos'] is None:
            raise Exception(
                "null key. " + str(interpolation_info) + "was passed to apply_interpolation_to_line_segment")
        return interpolate_within_line_segment(interpolation_info['line_seg'], interpolation_info['horizontal_pos'])

    line_segment_averaging_set = line_segment_averaging_set_iterable(line_segments_to_average)
    return number_average(line_segment_averaging_set, apply_interpolation_to_line_segment)
...
def get_representative_line_from_rotated_line_segments(trajectory_line_segments, min_vertical_lines, min_prev_dist):
    inputs = get_representative_trajectory_average_inputs(trajectory_line_segments=trajectory_line_segments,
                                                          ...
                                                          min_lines=min_vertical_lines)
    out = []
    for line_seg_averaging_input in inputs:
        vert_val = get_mean_vertical_coordinate_in_line_segment(line_seg_averaging_input)
...
def get_representative_line_from_trajectory_line_segments(trajectory_line_segments, min_vertical_lines, min_prev_dist):
    average_trajectory_vector = get_average_vector(
        line_segment_list=list(map(lambda x: x.line_segment, trajectory_line_segments)))
    ...
    representative_points = get_representative_line_from_rotated_line_segments(
        trajectory_line_segments=trajectory_line_segments,
        min_vertical_lines=min_vertical_lines,
        min_prev_dist=min_prev_dist)
    return list(map(lambda x: x.rotated(angle_in_degrees=average_trajectory_vector.angle), representative_points))
